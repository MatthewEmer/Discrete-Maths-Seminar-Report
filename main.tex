...
\separate{5}\textbf{Example 2.1.1}: We can prove that, for any \(n \geq 1, n \in \N,\)
...
\definition{4.1}{\underline{Young diagrams} represent partitions by stacking rows of \\ squares (corresponding to individual parts) on top of each other, in \\ ascending order (See Figure \ref{fig:visualisationOptions}(a)).}

\separate{5}\definition{4.2}{\underline{Ferrers graphs} represent partitions in a similar way, but instead electing to utilise dots instead of squares when representing parts \\(See Figure \ref{fig:visualisationOptions}(b)).} 
